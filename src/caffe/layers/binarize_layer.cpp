...
template <typename Dtype>
void BinarizeLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  const int count = top[0]->count();
	const Dtype* bottom_data = bottom[0]->cpu_data();
  Dtype* top_data = top[0]->mutable_cpu_data();
	caffe_cpu_sign(count, bottom_data, top_data);
}
...
void BinarizeLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    Dtype* bottom_diff = bottom[0]->mutable_cpu_diff();
		...
		{
			//Dtype tanhx = tanh(bottom_data[i]);
			//bottom_diff[i] = top_diff[i] * (1 - tanhx * tanhx);
			bottom_diff[i] = top_diff[i] * (1 - abs(bottom_data[i]));
		}
