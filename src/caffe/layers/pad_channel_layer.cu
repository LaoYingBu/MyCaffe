...
namespace caffe {
	...
	__global__ void pad_forward_kernel(const int dst_count, const int bottom_channels, const int top_channels,
		const int dim, const Dtype* src, Dtype* dst)
	{
		CUDA_KERNEL_LOOP(index, dst_count)
		{
			int num = index / (dim * top_channels);
			int dst_c = index / dim % top_channels;
			int pixel_pos = index % dim;
			if (dst_c < bottom_channels)
				dst[index] = src[num * bottom_channels * dim + dst_c * dim + pixel_pos];
			else
				dst[index] = Dtype(0);
		}
	}
