...
void LiftedStructuredLossLayer<Dtype>::LayerSetUp(
...
template <typename Dtype>
void LiftedStructuredLossLayer<Dtype>::Forward_cpu(
	...
	for (int di = 0; di < batch_size; di++)
	{
		...
		{
			if (k / cluster_size == c) continue;
			sum_exp += exp(theta * (margin + min_dist_n - dist_data[di * batch_size + k]));
		}

		Dtype coeff_exp = 0;
		for (int dj = c * cluster_size; dj < (c + 1) * cluster_size; dj++)
		{
			if (di == dj) continue;
			
			Dtype lossk = log(sum_exp) / theta - min_dist_n + dist_data[di * batch_size + dj];
			if (lossk > 0)
			{
				loss += lossk;
				vec_diff_coeff[di * batch_size + di] += 1;
				vec_diff_coeff[di * batch_size + dj] -= 1;				
				vec_diff_coeff[dj * batch_size + dj] += 1;
				vec_diff_coeff[dj * batch_size + di] -= 1;
				coeff_exp++;
			}
		}		
		if (coeff_exp > 0)
		{
			Dtype dtheta = coeff_exp * (min_dist_n - log(sum_exp) / theta) / theta;
			for (int k = 0; k < batch_size; k++)
			{
				if (k / cluster_size == c) continue;
				Dtype d = -coeff_exp * exp(theta * (margin + min_dist_n - dist_data[di * batch_size + k])) / sum_exp;
			...
			this->blobs_[0]->mutable_cpu_diff()[0] += dtheta;
		}		
	}

	int valid_triplet = num_clusters * cluster_size * (cluster_size - 1);
	top[0]->mutable_cpu_data()[0] = loss / 2 / valid_triplet;
	this->blobs_[0]->mutable_cpu_diff()[0] /= valid_triplet;
	caffe_scal(vec_diff_coeff_.count(), Dtype(1) / valid_triplet, vec_diff_coeff_.mutable_cpu_data());
}

template <typename Dtype>
void LiftedStructuredLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(LiftedStructuredLossLayer);
#endif

INSTANTIATE_CLASS(LiftedStructuredLossLayer);
REGISTER_LAYER_CLASS(LiftedStructuredLoss);
