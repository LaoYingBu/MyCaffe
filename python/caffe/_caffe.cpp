...
/*******************************##3 blobs##*****************************************/
void Net_SetInputArrays_3blobs(Net<Dtype>* net, bp::object data_obj,
	bp::object labels_obj, bp::object weights_obj) {
	// check that this network has an input MemoryDataLayer
	shared_ptr<MemoryDataWeightLayer<Dtype> > md_layer =
		boost::dynamic_pointer_cast<MemoryDataWeightLayer<Dtype> >(net->layers()[0]);
	if (!md_layer) {
		throw std::runtime_error("set_input_arrays may only be called if the"
			" first layer is a MemoryDataLayer");
	}
	...
	if (((PyArray_DIMS(data_arr)[0] != PyArray_DIMS(labels_arr)[0])) && ((PyArray_DIMS(data_arr)[0] != PyArray_DIMS(weights_arr)[0]))) {
		throw std::runtime_error("data and labels must have the same first"
			" dimension");
	}
	if (PyArray_DIMS(data_arr)[0] % md_layer->batch_size() != 0) {
...
BOOST_PYTHON_MODULE(_caffe) {
  ...
  bp::class_<Net<Dtype>, shared_ptr<Net<Dtype> >, boost::noncopyable >("Net",
	...
	.def("_set_input_arrays_3blobs", &Net_SetInputArrays_3blobs,
		bp::with_custodian_and_ward<1, 2, bp::with_custodian_and_ward<1, 3> >())
    .def("save", &Net_Save);
