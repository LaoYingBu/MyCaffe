...
################3 blobs#############
def _Net_set_input_arrays_3blobs(self, data, labels,weights):
    """
    Set input arrays of the in-memory MemoryDataLayer.
    (Note: this is only for networks declared with the memory data layer.)
    """
    ...
    if weights.ndim == 1:
        weights = np.ascontiguousarray(weights[:, np.newaxis, np.newaxis,
                                             np.newaxis])
    return self._set_input_arrays_3blobs(data, labels,weights)
...
Net.set_input_arrays_3blobs = _Net_set_input_arrays_3blobs
